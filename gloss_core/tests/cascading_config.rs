...
use gloss_core::{generate_for_project, Config, PathMode};
...
file_pattern = "{module}_root.gleam"
encoder_pattern = "root_encode_{module}.gleam"
decoder_pattern = "root_decode_{module}.gleam"
"#,
    )
...
encoder_pattern = "api_encode_{module}.gleam"
decoder_pattern = "api_decode_{module}.gleam"
"#,
    )
...
"#,
    ...
    fs::write(
        ...
        r#"
// gloss-file!: output_dir = "./file_gen", separate_encoder_decoder = false, encoder_pattern = "file_encode_{module}.gleam", decoder_pattern = "file_decode_{module}.gleam", file_pattern = "file_combined_{module}.gleam"
...
"#,
    ...
    let base_config = Config::load_or_default(&root);
    let generated = generate_for_project(&root, &base_config).expect("generate project");
    ...
    assert_eq!(
        account_code.output_config.encoder_pattern,
        "api_encode_{module}.gleam"
    );
    assert_eq!(
        account_code.output_config.decoder_pattern,
        "api_decode_{module}.gleam"
    );
    assert_eq!(
        account_code.output_config.file_pattern,
        "{module}_root.gleam"
    );
    ...
    assert_eq!(
        user_code.output_config.encoder_pattern,
        "file_encode_{module}.gleam"
    );
    assert_eq!(
        user_code.output_config.decoder_pattern,
        "file_decode_{module}.gleam"
    );
    assert_eq!(
        user_code.output_config.file_pattern,
...
directory = "@/global_gen"
separate_files = true
file_pattern = "{module}_global.gleam"
"#,
    )
    ...
    fs::write(
        ...
        r#"
// gloss-file!: output_dir = "@/team_file_gen", decoder_pattern = "file_decode_{module}.gleam"
...
// gloss!: encoder(json), decoder, output_dir = "./local_override", encoder_pattern = "local_encode_{module}.gleam", decoder_pattern = "local_decode_{module}.gleam"
pub type Leader {
  Leader(name: String)
}
"#,
    ...
    assert_eq!(
        member_group.output_config.decoder_pattern,
        "file_decode_{module}.gleam"
    );
    ...
    assert_eq!(
        leader_group.output_config.encoder_pattern,
        "local_encode_{module}.gleam"
    );
    assert_eq!(
        leader_group.output_config.decoder_pattern,
